(function () {
  const i = "https://chromewebstore.google.com/detail/kortex/hdapplggdhndkblofffknpmnnnnbncbn"; function d() { return window.location.pathname.split("/").pop() || "index.html" } function m(t) {
    const e = o => t === o ? "text-emerald-600" : "text-slate-500 hover:text-slate-900", r = t === "index.html" && window.location.hash === "#pricing" ? "text-emerald-600" : "text-slate-500 hover:text-slate-900", n = o => typeof chrome < "u" && chrome.runtime && chrome.runtime.getURL ? o : o === "index.html" ? "/" : "/" + o.replace(".html", ""), l = typeof chrome < "u" && chrome.runtime && chrome.runtime.getURL ? chrome.runtime.getURL("icons/icon128.png") : "/icons/icon128.png"; return `
      <header id="navbar" class="fixed left-1/2 -translate-x-1/2 z-50 w-[92%] max-w-6xl rounded-2xl border border-slate-200/60 flex items-center justify-between px-6 bg-white/70 backdrop-blur-xl shadow-2xl shadow-slate-200/50 py-2 top-4">
        <nav class="flex items-center justify-between w-full">
          <a href="${n("index.html")}" class="flex items-center gap-2 font-black text-xl tracking-tighter group cursor-pointer">
          ...
          <div class="hidden md:flex items-center gap-1 bg-slate-100/50 p-1 rounded-full border border-slate-200/40 text-xs font-bold">
            ...
            <a href="${n("index.html")}#pricing" class="px-5 py-2 ${r} hover:bg-white rounded-full transition-all">Pricing</a>
